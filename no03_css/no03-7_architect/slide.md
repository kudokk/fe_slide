...
CSS 設計がないと、大規模なサイトを複数人で運用するときに、書き方に一貫性が無くなるためです。
...
Aさんは以下のようのボタンを表現したとします。
html
```html
<div class="btn">送信ボタン</div>
```
...
<p class="-small">一方、Bさんは以下のようにボタンを表現したとします。</p>

```html
<div class="button is-blue">送信ボタン</div>
```
...
<p class="-small">このように二種類の同じようなコードがある状態では、次の担当者の C さんは A さんのコードをお手本に次の CSS 書けば良いのか、分からなくなってしまいます。<br>
<span class="-b -u">そして、コード量が無駄に増え、どのCSSプロパティが優先されるのか分かりづらいblackbox状態になってしまいます。</span>
このような状況を避けるために CSS 設計は存在します。</p>
...
あくまで１例になりますが、レッツエンジョイ東京のモジュールをBEMで実装してみました。  
